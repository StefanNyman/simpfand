...
unsigned short get_temp() {
  ...
  if (!fp)
    die("error: could not read temperature input", EXIT_FAILURE);
...
unsigned short get_level(unsigned short curr_temp, int dir,
                         struct config *cfg) {
  unsigned level = INIT_GARBAGE;

  if (dir == INC) {
    if (curr_temp < cfg->inc_lvls[0].tmp) {
      level = cfg->base_lvl;
    } else {
      for (int i = MAX_FAN_LVL - 1; i >= 0; i--) {
        if (curr_temp >= cfg->inc_lvls[i].tmp) {
          level = cfg->inc_lvls[i].lvl;
          break;
        }
      }
    }
  ...
  if (dir == DEC) {
    if (curr_temp < cfg->dec_lvls[0].tmp) {
      level = cfg->base_lvl;
    } else {
      for (int i = 1; i < MAX_FAN_LVL; i++) {
        if (curr_temp <= cfg->dec_lvls[i].tmp) {
          level = cfg->dec_lvls[i].lvl;
          break;
        }
      }
    }
...
int detect_change(unsigned short curr_temp, unsigned short prev_lvl, int dir,
                  struct config *cfg) {
